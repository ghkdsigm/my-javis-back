...
function isOurNewsListText(t) {
  const s = String(t || '');
  return /상위\s*\d+건입니다/.test(s) && /특정 기사를 들으시려면/.test(s);
}
...
export async function maybeEmitToolEvent(text, send) {
  ...
  if (hasAny(weatherKeyword, lower) && hasAny(weatherAction, lower) && !hasAny(exceptWords, lower)) {
    ...
    try {
      ...
      const payload = {
        ...
        text: w.text,  // 프론트는 tool.text를 표시/읽기
  ...
  if (hasAny(newsKeyword, lower) && hasAny(newsAction, lower) && !hasAny(exceptWords, lower)) {
    ...
    try {
      ...
      const maxShow = Number(process.env.NAVER_NEWS_DISPLAY || 5);
  ...
  if (/(읽어줘|자세히|상세|더 말해|읽어 봐|읽어봐|보여줘|자세히 보여줘|내용)/.test(lower)) {
    ...
    if (shouldDebounce('news.read', 1200)) return true;
  ...
  if (/(링크|클릭|열어줘|열어봐|이동)/.test(lower)) {
    ...
    if (shouldDebounce('news.open_url', 1000)) return true;
...
function extractKeywordForPick(utter) {
  const t = String(utter || '').trim();
  return t
    .replace(/(첫.?번|두.?번|세.?번|네.?번|다섯.?번|여섯.?번|일곱.?번|여덟.?번|아홉.?번|열.?번|[0-9]+ ?번|[0-9]+ ?번째|첫.?번째|두.?번째|세.?번째|네.?번째|다섯.?번째|여섯.?번째|일곱.?번째|여덟.?번째|아홉.?번째|열.?번째|마지막|읽어줘|읽어봐|상세|자세히|그 중|그중|그거|요약|내용|보여줘|링크|클릭|열어줘|열어봐|이동)/gi, '')
    .trim();
}
